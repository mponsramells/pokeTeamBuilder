  ...
  <div class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
    <div class="bg-white rounded-xl shadow-lg overflow-y-auto w-2/4 p-6 flex flex-col">
      <div class="flex justify-end mb-4">
        <button class="text-gray-600 font-bold p-2" @click="$emit('close-pokemon')">X</button>
      </div>
      <div class="flex-shrink-0">
        <img class="h-64 w-full object-contain" :src="pokemon.image" :alt="pokemon.name"/>
      </div>
      <div class="flex flex-col gap-3 w-full">
        <h2 class="text-xl font-semibold text-gray-800">{{ pokemon.name }}</h2>
        <p class="text-gray-600">Pokedex ID: #{{ pokemon.pokedexId }}</p>
        <div class="flex gap-2 flex-wrap">
          <span v-for="type in pokemon.apiTypes" :key="type.name"
                class="bg-gray-200 text-gray-700 rounded-full px-3 py-1 text-sm flex items-center gap-1">
